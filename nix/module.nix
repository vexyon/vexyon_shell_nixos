...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Batería. services/Battery.qml lee UPower.displayDevice (percentage,
    # state, timeToEmpty/timeToFull) por D-Bus, y de ese singleton cuelgan el
    # widget de la barra (selfHide si !present), el BatteryPanel entero y la
    # pastilla del LockScreen. Sin este servicio NO hay org.freedesktop.UPower
    # en el bus: `present` se queda en false, el widget se auto-oculta y el
    # panel dice "No battery" al 100% — que es exactamente el síntoma en un
    # portátil real.
    #
    # En Arch nadie lo pide explícitamente (install.sh no lista upower): llega
    # de rebote por dependencias de otros paquetes y el servicio ya está ahí.
    # Esa suposición es justo la que se rompe en un NixOS mínimo, donde nada
    # tira de él. Es D-Bus-activado (Type=dbus): no hay proceso vivo hasta que
    # el shell pregunta, y las actualizaciones llegan por señal — cero polling,
    # el mismo mecanismo que en Arch.
    services.upower.enable = true;

    # Perfiles de energía (Ahorro / Equilibrado / Rendimiento). Los usa
    # BatteryPanel.qml: lee el actual con `powerprofilesctl get` y cambia con
    # `powerprofilesctl set <perfil>`. Sin el servicio no hay bus al que
    # preguntar, el CLI ni siquiera está instalado, y la sección de perfiles del
    # panel no aparece.
    #
    # CORRIGE LO QUE DIJO LA SESIÓN 70. Allí se dio por bueno que "Arch tampoco
    # lo tiene" mirando solo la lista de paquetes de install.sh. Falso: en la
    # torre real (CachyOS) el servicio está activo y `powerprofilesctl get`
    # responde `balanced`. Llega igual que upower — de rebote, no desde
    # install.sh —, y como en NixOS no hay defaults implícitos, aquí hay que
    # pedirlo. Mismo hueco que upower y udisks2, misma solución.
    #
    # El paquete trae el CLI y el daemon juntos, y el módulo de nixpkgs lo mete
    # en environment.systemPackages, así que `powerprofilesctl` queda en el PATH
    # de la sesión sin añadirlo a mano. Activado por D-Bus
    # (org.freedesktop.UPower.PowerProfiles): sin proceso vivo hasta la primera
    # consulta y sin sondeo alguno.
    #
    # mkDefault porque nixpkgs afirma que power-profiles-daemon es incompatible
    # con services.tlp y services.auto-cpufreq: Vexyon no activa ninguno de los
    # dos, pero quien use TLP en su configuración puede apagarlo aquí sin pelear
    # con el módulo (perdiendo el selector de perfiles, que es su decisión).
