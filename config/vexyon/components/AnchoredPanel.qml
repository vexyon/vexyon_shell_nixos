...
PanelWindow {
    ...
    // Pantalla del widget que lo abrió (un clic en la barra de DP-1 abre en
    // DP-1, que es lo correcto). Si no hay anclaje válido —abierto por atajo o
    // IPC— se cae al monitor ENFOCADO, no a la primera pantalla: si no, el
    // panel reaparecía donde se abrió la última vez.
    screen: {
        var scs = Quickshell.screens;
        for (var i = 0; i < scs.length; i++)
            if (scs[i].name === Panels.anchorScreen) return scs[i];
        return Panels.focusedScreen();
    }
