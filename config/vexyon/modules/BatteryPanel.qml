...
//  Brightness/Audio de Vexyon + PowerProfiles (D-Bus a power-profiles-daemon).
// ============================================================================
AnchoredPanel {
    id: bp
    ...
    content: Component {
        Item {
            ...
            // ---- perfil de energía -----------------------------------------
            // PowerProfiles is Quickshell's binding to power-profiles-daemon's
            // D-Bus interface (org.freedesktop.UPower.PowerProfiles, a.k.a. the
            // older net.hadess.PowerProfiles). It is the VENDOR-AGNOSTIC layer:
            // reading `profile` gives the active profile and the property
            // re-evaluates by itself whenever the daemon signals a change --
            // so a laptop's own profile key (Fn+Q on a Legion, whatever the
            // equivalent is elsewhere) moves this pill for free, as long as the
            // vendor kernel driver feeds the daemon. Writing it does SetProfile.
            // Event-driven: no Process, no poller, no timer.
            //
            // WAS: a Process running `powerprofilesctl get` plus an execDetached
            // `powerprofilesctl set`, with the click RESTARTING the poller. That
            // was the dead-buttons bug: `set` is asynchronous, so the re-poll
            // came back with the OLD value (and with no powerprofilesctl on
            // PATH it always fell back to the literal "balanced") and clobbered
            // the user's choice in the same frame -- the pill never even moved.
            readonly property int powerProfile: PowerProfiles.profile
            readonly property color profileStart: {
                if (powerProfile === PowerProfile.Performance) return Theme.red;
                if (powerProfile === PowerProfile.PowerSaver) return Theme.green;
                return Theme.blue;
            }
                    ...
                    ColumnLayout {
                        ...
                        Rectangle {
                            ...
                            Rectangle {
                                ...
                                y: 1
                                radius: 10
                                x: body.powerProfile === PowerProfile.Performance ? 1
                                 : body.powerProfile === PowerProfile.Balanced ? width + 1
                                 : (width * 2) + 1
                            ...
                            RowLayout {
                                ...
                                Repeater {
                                    model: [
                                        { profile: PowerProfile.Performance, icon: "󰓅", label: I18n.t("Perf.") },
                                        { profile: PowerProfile.Balanced,    icon: "󰗑", label: I18n.t("Balanced") },
                                        { profile: PowerProfile.PowerSaver,  icon: "󰌪", label: I18n.t("Saving") }
                                    ]
                                    delegate: Item {
                                        ...
                                        RowLayout {
                                            ...
                                            Text {
                                                ...
                                                color: body.powerProfile === modelData.profile ? Theme.crust : (profileMa.containsMouse ? Theme.text : Theme.subtext0)
                                                text: modelData.icon
                                                Behavior on color { ColorAnimation { duration: 200 } }
                                            }
                                            Text {
                                        ...
                                        MouseArea {
                                            ...
                                            // No local state to set: the pill follows
                                            // PowerProfiles.profile, which the daemon
                                            // updates back over D-Bus.
