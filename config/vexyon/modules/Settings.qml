...
FloatingWindow {
    ...
    // Petición de "ábrete en ESTA página" (Panels.openSettingsAt). Se atiende
    // en cuanto cambia, así que da igual que la ventana estuviera ya abierta:
    // poner Panels.settings a true otra vez no emitiría ninguna señal.
    Connections {
        target: Panels
        function onSettingsPageChanged() {
            if (Panels.settingsPage === "") return;
            win.current = Panels.settingsPage;
            win.query = "";                 // una búsqueda a medias taparía la página
            Panels.settingsPage = "";       // consumida
        }
    }
