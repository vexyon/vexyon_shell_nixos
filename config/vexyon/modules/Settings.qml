            ...
            Flickable {
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        // ---- rotación ------------------------------------------------
                        //  Un monitor girado 90°/270° ocupa en pantalla el ALTO por el
                        //  ANCHO. `hyprctl monitors -j` NO lo refleja: sigue reportando
                        //  la resolución del MODO (medido en vivo: con transform = 1
                        //  devuelve 1280×800, no 800×1280), así que el giro hay que
                        //  aplicarlo aquí. Impares = 90°/270° (1 y 3) y sus variantes
                        //  volteadas (5 y 7); 0/2/4/6 conservan la orientación.
                        //
                        //  El transform se lee de la CONFIG (igual que x/y), no del
                        //  monitor vivo, para que la caja gire en cuanto se elige en el
                        //  desplegable y no un refresco después.
                        //
                        //  lw()/lh() son la única fuente de tamaño lógico: las usan la
                        //  caja dibujada Y el imán del arrastre, así que arreglar esto
                        //  arregla las dos cosas a la vez — un monitor vertical se
                        //  arrastra y se pega por su silueta real, no por la apaisada.
                        function tf(m) {
                            var c = Displays.monCfg(m.name);
                            return (c && c.transform !== undefined ? c.transform : (m.transform || 0)) || 0;
                        }
                        function rot(m) { return (canvas.tf(m) % 2) === 1; }
                        function lw(m) { return Math.max(1, Math.round((canvas.rot(m) ? m.height : m.width) / m.scale)); }
                        function lh(m) { return Math.max(1, Math.round((canvas.rot(m) ? m.width : m.height) / m.scale)); }
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        Layout.alignment: Qt.AlignHCenter
                                        // Resolución del MODO + el giro cuando lo hay, para
                                        // que se entienda por qué la caja está de pie.
                                        text: box.modelData.width + "×" + box.modelData.height
                                              + (canvas.tf(box.modelData) !== 0
                                                 ? " · " + Displays.transformLabel(canvas.tf(box.modelData)) : "")
