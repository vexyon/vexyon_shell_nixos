...
Item {
    ...
    //  ninguna (o con el interruptor de Virtualización apagado) `selfHide` la
    //  borra de la barra entera — el mismo mecanismo exacto que usa la pastilla
    //  de batería en un sobremesa (`selfHide: !Battery.present`), no un
    //  invento nuevo.
    //
    //  CÓMO SABE SI HAY ALGUNA ENCENDIDA, SIN SONDEAR NADA: `Vm.anyRunning` es
    //  una propiedad DERIVADA de la caché de `Vm.vms`; leerla no lanza ningún
    //  proceso. Esa caché se rellena UNA vez al crearse la pastilla (`prime()`,
    //  por si una VM llevaba encendida desde antes de arrancar el shell) y a
    //  partir de ahí se actualiza sola tras CADA acción del usuario, porque
    //  Vm.run() siempre relee al terminar. No hay ningún Timer en ninguna parte
    //  de este camino.
    Component { id: cVm
        Pill {
            readonly property bool selfHide: !Vm.enabled || !Vm.anyRunning
            Component.onCompleted: Vm.prime()
            glyph: Icons.desktop
            glyphColor: {
                var r = Vm.runningVms;
                for (var i = 0; i < r.length; i++) if (r[i].state === "paused") return Theme.yellow;
                return Theme.green;
            }
            // El número solo aparece con más de una encendida; con una sola la
