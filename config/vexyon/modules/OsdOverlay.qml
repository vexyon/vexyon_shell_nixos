...
Variants {
    ...
    PanelWindow {
        ...
        // Solo en el monitor ENFOCADO. Antes se dibujaba en los tres a la vez
        // (una PanelWindow por pantalla): el usuario veía el OSD también en
        // monitores donde no estaba mirando.
        readonly property bool onFocused:
            Hyprland.focusedMonitor ? modelData.name === Hyprland.focusedMonitor.name : true
        // keep the surface mapped through the exit animation, unmap when done
        visible: onFocused && card.opacity > 0.01
