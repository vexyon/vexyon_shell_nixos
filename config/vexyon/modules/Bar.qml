        ...
        Item {
            ...
            WheelHandler {
                ...
                onWheel: function(e) {
                    ...
                    // Raíz Lua: el argumento de `dispatch` se evalúa como una
                    // EXPRESIÓN Lua, así que la cadena clásica "workspace e+1"
                    // muere en el parser (')' expected near 'e'). El selector
                    // relativo sigue siendo el mismo, ahora como string dentro
                    // del dispatcher — misma forma que genera el bridge.
                    Hyprland.dispatch('hl.dsp.focus({ workspace = "e'
                                      + (down ? "+" : "-") + '1" })');
                }
            }
