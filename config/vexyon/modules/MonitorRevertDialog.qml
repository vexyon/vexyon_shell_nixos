...
PanelWindow {
    ...
    // Monitor con el foco cuando salta el diálogo. Se FIJA al aparecer, no es
    // un binding vivo: el diálogo no debe saltar de pantalla mientras corre la
    // cuenta atrás. `visible` espera a tener el nombre para que la ventana no
    // llegue a mapearse un instante en la pantalla por defecto.
    property string scrName: ""
    Connections {
        target: Displays
        function onPendingRevertChanged() {
            win.scrName = Displays.pendingRevert ? Panels.focusedScreenName() : "";
        }
    }
    screen: Panels.screenFor(win.scrName)
    visible: Displays.pendingRevert && win.scrName !== ""
