...
PanelWindow {
    ...
    // ---- entradas propias del shell -----------------------------------------
    //  Ventanas que no son apps de escritorio y por tanto no tienen .desktop
    //  que DesktopEntries pueda encontrar. Se listan como una entrada más y se
    //  puntúan/filtran con el mismo código que el resto (llevan name/comment/
    ...
    function shellEntries() {
        ...
        return [ {
            ...
            genericName: I18n.t("Virtual machine manager"),
            comment: I18n.t("Create and control libvirt/QEMU virtual machines"),
    ...
    function rebuild() {
        ...
        var apps = win.shellEntries().concat(win.allApps);
        var scored = [];
    ...
    function launch(entry) {
        if (!entry) return;
        // Entrada propia del shell: abre su ventana, no lanza un proceso.
        if (entry.shellPanel) { Panels.close("launcher"); Panels.open(entry.shellPanel); return; }
        try { entry.execute(); }
