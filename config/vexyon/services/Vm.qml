...
Singleton {
    ...
    function start(name)      { root.run("start",    "$V start "    + q(name)); }
    ...
    function attachExistingDisk(name, path, bus) {
        var pre = bus === "virtio" ? "vd" : "sd";
    ...
    function insertCd(name, target, iso) {
        root.run("cd-insert",
            root.cfgLive(name,
    ...
    function addSharedFolder(name, path, tag) { root.run("fs-add", root.editXml(name, "fs-add " + q(path) + " " + q(tag))); }
    ...
    function exportVm(name, path) {
        root.run("export", "$V dumpxml --inactive " + q(name) + " > " + q(path));
    }
    function importVm(path) {
        root.run("import", "$V define " + q(path));
    }
