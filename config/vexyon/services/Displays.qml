...
Singleton {
    ...
    Component.onCompleted: { root.refresh(); root._liveWatch = true; }

    // ---- refresco en vivo de la topología ---------------------------------
    //  `Component.onCompleted` a secas dejaba la lista CONGELADA: conectar,
    //  desconectar o girar un monitor con la página de Pantallas abierta no
    //  movía el gráfico hasta salir y volver a entrar (era el único llamador de
    //  refresh(), en Settings.qml).
    //
    //  La fuente es `Quickshell.screens` (o sea wl_output), NO los eventos de
    //  Hyprland, y eso está medido en la VM, no supuesto: Hyprland emite
    //  `monitoradded`/`monitorremoved`, pero al cambiar transform, modo o
    //  posición **no emite absolutamente nada**, así que un giro hecho por
    //  `hyprctl` desde fuera nunca se enteraría. wl_output sí propaga los tres
    //  casos (un giro intercambia width/height del QsScreen).
    //
    //  Se compara una FIRMA de la topología, no una señal cruda: Qt solo emite
    //  el cambio cuando el string difiere de verdad, así que el rebote de
    //  eventos redundantes se colapsa en un único refresco (Hyprland manda
    //  `monitorremoved` tres veces seguidas por una sola desconexión). Sin
    //  temporizador y sin sondeo: el binding solo se reevalúa cuando el
    //  compositor cambia una salida.
    property bool _liveWatch: false
    readonly property string topologySignature: {
        var scs = Quickshell.screens, out = [];
        for (var i = 0; i < scs.length; i++)
            out.push(scs[i].name + ":" + scs[i].x + "," + scs[i].y
                     + ":" + scs[i].width + "x" + scs[i].height
                     + ":" + scs[i].devicePixelRatio);
        return out.join("|");
    }
    // El guard evita el refresco doble del arranque: la firma se evalúa por
    // primera vez ANTES del Component.onCompleted de este singleton, que es
    // quien hace la carga inicial.
    onTopologySignatureChanged: if (root._liveWatch) root.refresh()
    ...
    // etiqueta de un transform (0..7) según la tabla de arriba
    function transformLabel(v) {
        for (var i = 0; i < root.transforms.length; i++)
            if (root.transforms[i].v === v) return root.transforms[i].l;
        return "";
    }
