...
Singleton {
                ...
                try {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        out.push({
                            ...
                            // full EDID description (make + model + serial) — the
                            // stable identity used for default-workspace binding.
    ...
    // ============== default workspace per monitor (stable-keyed) ============
    //  El usuario elige qué workspace es el "de casa" de cada monitor. La clave
    //  NO puede ser el conector (DP-1/DP-2/DP-3): en la torre real el orden de
    //  puerto no coincide con el orden físico (DP-1 izquierda, DP-3 centro,
    //  DP-2 derecha) y un reenchufe puede reordenarlos. Usamos la `description`
    //  del EDID (marca + modelo + nº de serie), que es lo único estable entre
    //  reinicios y reconexiones — y lo único que distingue dos paneles del mismo
    //  modelo (la torre tiene dos GIGA-BYTE GS27FA idénticos salvo serie).
    //
    //  Se guarda en shell.json displays.defaultWorkspaces = { "<desc>": n },
    //  fuera de los perfiles: la asignación es del monitor físico, no del perfil
    //  ni del puerto. El bridge la convierte en hl.workspace_rule(...).

    // clave estable de un monitor (description; conector solo como último recurso)
    function stableId(mon) {
        if (!mon) return "";
        return (mon.description && mon.description !== "") ? mon.description : mon.name;
    }

    function defaultWorkspaces() { return Config.get("displays", "defaultWorkspaces", {}) || ({}); }

    // workspace asignado a un monitor, o 0 si ninguno
    function defaultWorkspaceFor(mon) {
        var m = root.defaultWorkspaces();
        var v = m[root.stableId(mon)];
        return (v === undefined || v === null) ? 0 : Number(v);
    }

    // opciones del selector: "Automático" (0) + 1..workspaceCount
    function workspaceOptions() {
        var n = Config.get("layout", "workspaceCount", 10);
        n = Math.max(1, Math.min(Number(n) || 10, 10));
        var o = [ { v: 0, l: I18n.t("Automatic") } ];
        for (var i = 1; i <= n; i++) o.push({ v: i, l: String(i) });
        return o;
    }

    // Asigna (ws = 0 -> quitar). Dos monitores con el MISMO número serían
    // ambiguos (Hyprland aplicaría la última regla y el otro monitor se quedaría
    // sin su workspace "de casa" en silencio), así que lo evitamos en origen:
    // asignar un número que ya tenía otro monitor se lo quita a ese otro.
    // Regla documentada: gana el último asignado.
    function setDefaultWorkspace(mon, ws) {
        var key = root.stableId(mon);
        if (key === "") return;
        var m = JSON.parse(JSON.stringify(root.defaultWorkspaces()));
        ws = Number(ws) || 0;
        if (ws === 0) delete m[key];
        else {
            for (var k in m) if (k !== key && Number(m[k]) === ws) delete m[k];
            m[key] = ws;
        }
        Config.set("displays", "defaultWorkspaces", m);
        root.applyDefaultWorkspaces();
    }

    // Aplica en vivo todas las reglas guardadas, solo para que el cambio se note
    // al instante (el bridge tarda hasta POLL_INTERVAL en regenerar + recargar).
    // Una regla cuyo monitor no está conectado se acepta sin error y sencillamente
    // no casa con nada (verificado en Hyprland 0.55.4), así que emitimos el mapa
    // entero: al reconectar el monitor la asignación vuelve sola.
    //
    // Quitar una asignación NO se puede hacer en vivo (no hay "unset rule"), pero
    // no hace falta: `hyprctl reload` borra todas las workspace rules y las relee
    // del .lua regenerado (verificado), así que el bridge cierra ese caso.
    Process { id: wsRuleApplier }
    function applyDefaultWorkspaces() {
        var m = root.defaultWorkspaces();
        var calls = [];
        for (var k in m) {
            var ws = Number(m[k]);
            if (!ws) continue;
            calls.push(root.wsRuleCall(k, ws));
        }
        if (calls.length === 0) return;
        wsRuleApplier.command = ["hyprctl", "eval", calls.join(" ")];
        wsRuleApplier.running = true;
    }

    // Mirrors lua_workspace_rule() in the bridge.
    function wsRuleCall(key, ws) {
        // desc: hace match por descripción EDID; un conector suelto (fallback)
        // se pasa tal cual.
        var sel = (key.indexOf("-") > 0 && key.indexOf(" ") < 0) ? key : "desc:" + key;
        return 'hl.workspace_rule({ workspace = "' + ws + '", monitor = "'
             + sel.replace(/"/g, '\\"') + '", default = true })';
    }
    ...
    //  Tras aplicar en vivo, releer el estado real. Sin esto la lista `monitors`
    //  se quedaba con lo de antes y la UI de Pantallas (en particular el croquis
    //  de Disposición, que ahora dibuja la ROTACIÓN) no se enteraba del cambio
    //  hasta reabrir la página. Es por EVENTO — se dispara al terminar el
    //  hyprctl, no hay sondeo. autoApply() está guardado por fingerprint, así
    //  que este refresh no puede realimentarse.
    Process { id: applier; onExited: root.refresh() }
