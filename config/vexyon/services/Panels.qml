...
Singleton {
    ...
    // Virtualización. `vmManager` es una VENTANA de verdad (FloatingWindow),
    // como Ajustes: vive FUERA del conjunto excluyente de popups y por eso
    ...
    // ---- abrir Ajustes DIRECTAMENTE en una página --------------------------
    //  Antes solo se podía pedir "abre Ajustes" (Panels.settings = true) y el
    //  usuario aterrizaba donde lo hubiera dejado la última vez. Quien manda a
    //  Ajustes por un motivo CONCRETO ("aquí es donde se enciende esto")
    //  necesita dejarlo en esa página o el atajo no lleva a ningún sitio.
    //
    //  Es una petición, no un estado: Ajustes la consume y la vacía. Por eso
    //  vale igual con la ventana ya abierta (el cambio de propiedad se ve) que
    //  cerrada (se abre y va). No hay navegador general ninguno: una cadena.
    property string settingsPage: ""
    function openSettingsAt(page) {
        root.openScreen = root.focusedScreen();
        root.settingsPage = page;      // Ajustes lo lee y lo vacía
        root.settings = true;
    }
    ...
    readonly property var anchoredPanels: [
        ...
        "clipboardPanel", "themeQuick", "vmPanel"
    ]
