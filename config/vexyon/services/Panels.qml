...
Singleton {
    ...
    // ---- ¿en qué monitor sale un overlay? -----------------------------------
    //  ÚNICA fuente de verdad. Un panel tiene que salir DONDE ESTÁ EL USUARIO.
    //  `Quickshell.screens[0]` NO es "donde está el usuario": es el primer
    //  output que enumera el compositor — DP-1 en la torre de 3 monitores —, y
    //  por eso los atajos (Super+C, Super+Alt+T, launcher, wallpaper, power
    //  menu) abrían SIEMPRE ahí. La verdad viva es `Hyprland.focusedMonitor`,
    //  que ya usaba ScreenshotOverlay y que es el mismo nombre de conector que
    //  trae `QsScreen.name`.
    //
    //  Se FIJA al abrir (anchorScreen) y NO se re-evalúa mientras el panel está
    //  en pantalla: un binding vivo a focusedMonitor movería de monitor un
    //  panel ya abierto en cuanto el foco cambiara — con focus_follows_mouse,
    //  con solo pasar el ratón por encima de otra pantalla.
    ...
    function screenFor(name) {
        ...
        for (var i = 0; i < scs.length; i++)
            if (scs[i].name === name) return scs[i];
    ...
    function openAt(name, item, screenName, barPos, barWindow, sectionItem, sectionAlign, sectionSplit) {
        ...
        // NO se llama a open(): open() fija anchorScreen al monitor con foco, y
        // aquí la pantalla ya la sabemos — es la de la barra que contiene el
        // widget, que es la correcta aunque el foco esté en otra.
        root.closeAll();
        root[name] = true;
    }

    // Abre/cierra un panel anclado desde un atajo de teclado (sin widget que
    // lo ancle): lo cuelga del extremo final de la barra EN EL MONITOR CON EL
    // FOCO (arriba a la derecha con barra top, etc.).
    function toggleFallback(name) {
        if (root[name] === true) { root[name] = false; return; }
        var scr = root.screenFor(root.focusedScreenName());
        if (scr) {
    ...
    // toggle()/open() son la vía de los ATAJOS (launcher, wallpaper, power
    // menu, screenshot): no hay widget que diga la pantalla, así que se fija la
    // del monitor con foco. Los paneles anclados no pasan por aquí — openAt()
    // pone su propia anchorScreen y abre directo.
    function toggle(name) {
        var was = root[name];
        closeAll();
        if (was) return;
        root.anchorScreen = root.focusedScreenName();
        root[name] = true;
    }
    function open(name) {
        closeAll();
